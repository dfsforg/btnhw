...
def der_to_raw_signature(der_signature):
    ...
    # Convert r and s to 32-byte integers
...
def SignTnx(raw_unsigned_tnx_hash,key_id,pin):
    print(key_id,pin)
    device = PicoHSM(pin=pin)
    signature=device.sign(keyid=key_id,data=raw_unsigned_tnx_hash,scheme=0x70)
    raw_signature=der_to_raw_signature(signature)
    return raw_signature
...
with open("./tmp/submit_unsigned_tnx_hash_bin", 'rb') as binary_file:
    submit_unsigned_tnx_hash_bin = binary_file.read()
...
signature = SignTnx(submit_unsigned_tnx_hash_bin,1,device_config.PIN)
print(len(signature))
